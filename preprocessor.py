...
class Preprocessor:
    ...
    def drop_columns(
        self, drop_columns_file_path: str = "drop_columns_0402.txt"
    ) -> None:
    ...
    def __preprocess_target_variable(
        self, target_variable: str = "loan_status"
    ) -> None:
        # loan_status가 "current", "issued", "policy" 인 행을 필터링하여 삭제
        modified_df = self.df[
            ~self.df[target_variable].isin(
                [
                    "Current",
                    "Issued",
        ...
        # risk = 1, safe = 0 으로 처리
        modified_df.loc[
            modified_df["loan_status"].isin(["Fully Paid", "In Grace Period"]),
            "loan_status",
        ] = 0
        modified_df.loc[
            modified_df["loan_status"].isin(
    ...
    def __fill_na_with_value(
        self, columns: List[str], filling_value: Union[str, int]
    ) -> None:
        """
    ...
    def __preprocessing_na(self) -> None:
        ...
        self.df["is_after_2015"] = self.df["all_util"].apply(
        ...
        self.df["is_after_2012"] = self.df["pct_tl_nvr_dlq"].apply(
            lambda x: 0 if pd.isnull(x) else 1
        )
    ...
    def __preprocessing_objects(self) -> None:
        ...
        # emp_length
        self.df["emp_length"] = self.df["emp_length"].apply(
            lambda x: x.replace(" years", "")
            .replace(" year", "")
            .replace("+", "")
            .replace("< 1", "0")
        )
        ...
        # revol_util
        self.df["revol_util"] = self.df["revol_util"].apply(
            self.__delete_suffix_percentage
        )
        ...
        ## one-hot
        # home_ownership
        self.df["home_ownership"] = self.df["home_ownership"].replace(
            ["ANY", "OTHER", "NONE"], "OTHERS"
        )
    ...
    def preprocess(self) -> None:
        ...
        self.df.dropna(
            subset=self.df.columns.difference(["loan_status"]), how="all", inplace=True
        )
